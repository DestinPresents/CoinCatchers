...
// Safe random number helper used throughout the game.
...
function startGame(){
    ...
    // Spawn immediately so the game never starts with an empty field.
    spawnObject();
    spawnObject();
    scheduleNextSpawn();
}
...
function gameLoop(timestamp){
    ...
    updateObjects(delta);
    gameState.elapsedTime += delta * (1000/60);
    checkLevelProgress();
...
function scheduleNextSpawn(){
    ...
    gameState.spawnTimer=setTimeout(()=>{
        ...
        spawnObject();
        scheduleNextSpawn();
    },Math.max(180,difficulty.spawnInterval));
}
...
function updateObjects(delta){
    ...
    for(let i=gameState.objects.length-1;i>=0;i--){
        ...
        // Use one coordinate system only: top/left.
...
function endGame(){
    ...
    playerData.stats.totalPlayTime += Math.max(0,Math.round(gameState.elapsedTime/1000));
    playerData.lives=gameState.currentLives;
