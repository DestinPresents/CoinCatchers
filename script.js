...
let gameState = {
    ...
    elapsedTime:0,
    basketYOffset:0,
    fastCoinChance:GAME_CONFIG.FAST_COIN_CHANCE
};
...
function savePlayerData(){
    try{
        ...
        // IndexedDB is a secondary persistent backup for WebView/PWA environments.
        clearTimeout(idbSaveTimer);
        idbSaveTimer=setTimeout(()=>saveToIndexedDB(payload),300);
    }catch(error){
...
function scheduleSave(){
    ...
    saveTimer=setTimeout(()=>savePlayerData(),150);
}

function saveToIndexedDB(payload){
    try{
        if(!window.indexedDB)return;
        const request=indexedDB.open("CoinCatcherDB",1);
        ...
        request.onsuccess=()=>{
            ...
            try{
                ...
                tx.objectStore("save").put(payload,"playerData");
        ...
        console.warn("IndexedDB backup unavailable:",error);
    }
}

function restoreFromIndexedDBIfNeeded(){
    if(localStorage.getItem(STORAGE_KEY)||!window.indexedDB)return;
    try{
        const request=indexedDB.open("CoinCatcherDB",1);
        request.onupgradeneeded=()=>{
            const db=request.result;
        ...
        request.onsuccess=()=>{
            ...
            try{
                ...
                const getRequest=tx.objectStore("save").get("playerData");
                getRequest.onsuccess=()=>{
                    if(getRequest.result){
                        try{
                            playerData=deepMerge(DEFAULT_DATA,JSON.parse(getRequest.result));
...
function createLobbyMusicButton(){
    ...
    button.addEventListener("click",async()=>{
        const audio=ensureLobbyMusic();
        if(audio.paused){
            try{
                await audio.play();
                playerData.settings.music=true;
                savePlayerData();
            }catch(error){
            ...
            audio.pause();
            audio.currentTime=0;
            playerData.settings.music=false;
            savePlayerData();
        }
        updateMusicButton();
    });
    if(getComputedStyle(screens.home).position==="static")screens.home.style.position="relative";
    screens.home.appendChild(button);
    musicButton=button;
    updateMusicButton();
}
...
function showScreen(screenName){
    ...
    createLobbyMusicButton();
    if(screenName!=="home")stopLobbyMusic();
    updateAllUI();
}
...
function startGame(){
    ...
    gameState.basketYOffset=Number(playerData.settings.basketYOffset||0);
...
function getDifficulty(level){
    ...
    if(level<=5){
        speedMultiplier=0.78+level*0.02;
        spawnInterval=1000-level*10;
        maxObjects=3;
        bombChance=4;
    }else if(level<=10){
        speedMultiplier=0.9+(level-5)*0.04;
        spawnInterval=950-(level-5)*12;
        maxObjects=3;
        bombChance=7;
    }else if(level<=20){
        speedMultiplier=1.1+(level-10)*0.035;
        spawnInterval=890-(level-10)*10;
        maxObjects=3;
        bombChance=10;
    }else if(level<=30){
        speedMultiplier=1.45+(level-20)*0.035;
        spawnInterval=790-(level-20)*7;
        maxObjects=4;
        bombChance=13;
    }else if(level<=40){
        speedMultiplier=1.8+(level-30)*0.04;
        spawnInterval=720-(level-30)*6;
        maxObjects=4;
        bombChance=16;
    }else if(level<=50){
        speedMultiplier=2.2+(level-40)*0.045;
        spawnInterval=650-(level-40)*5;
        maxObjects=5;
        bombChance=20;
    }else if(level<=60){
        speedMultiplier=2.65+(level-50)*0.05;
        spawnInterval=600-(level-50)*4;
        maxObjects=5;
        bombChance=24;
    }else if(level<=70){
        speedMultiplier=3.15+(level-60)*0.06;
        spawnInterval=560-(level-60)*4;
        maxObjects=6;
        bombChance=28;
    }else if(level<=80){
        speedMultiplier=3.75+(level-70)*0.07;
        spawnInterval=500-(level-70)*4;
        maxObjects=7;
        bombChance=35;
    }else if(level<=90){
        speedMultiplier=4.45+(level-80)*0.075;
        spawnInterval=460-(level-80)*4;
        maxObjects=8;
        bombChance=42;
    }else{
        speedMultiplier=5.2+(level-90)*0.085;
        spawnInterval=420-(level-90)*3;
        maxObjects=10;
        bombChance=50;
    }
    ...
    return {
        speedMultiplier,
        spawnInterval: Math.max(160,spawnInterval),
        maxObjects,
        bombChance
    };
}
...
function spawnObject(){
    ...
    gameState.objects.push({
        ...
        x,
        y:startY,
        // Most objects keep the normal speed. A small share of coins are the
        // already-faster type and get a modest extra speed boost.
        speed:(()=>{
            const normalVariation=randomBetween(.98,1.08);
            const isFastCoin=type==="coin" && Math.random()<gameState.fastCoinChance;
            const fastBoost=isFastCoin?randomBetween(1.28,1.48):1;
            return GAME_CONFIG.BASE_FALL_SPEED*difficulty.speedMultiplier*normalVariation*fastBoost;
        })(),
...
function handleObjectMiss(obj){
    ...
    if(penalty>0){
        const before=playerData.coins;
        playerData.coins=Math.max(0,playerData.coins-penalty);
        const actualLoss=before-playerData.coins;
        showFloatingScore(obj.x,obj.y,`-${actualLoss} 🪙`);
        gameState.combo=0;
        savePlayerData();
        updateAllUI();
    }

    // A missed collectible also breaks the safe catch streak.
    if(obj.type!=="bomb")gameState.safeStreak=0;
}
...
const BASKET_VERTICAL_LIMIT=70;
...
function applyBasketPosition(){
    ...
    gameState.basketYOffset=Math.max(-BASKET_VERTICAL_LIMIT,Math.min(BASKET_VERTICAL_LIMIT,Number(gameState.basketYOffset)||0));
    basket.style.left=`${basketX+basket.offsetWidth/2}px`;
    basket.style.transform=`translate(-50%, ${gameState.basketYOffset}px)`;
}

function setBasketVerticalOffset(value,save=true){
    gameState.basketYOffset=Math.max(-BASKET_VERTICAL_LIMIT,Math.min(BASKET_VERTICAL_LIMIT,Number(value)||0));
    playerData.settings.basketYOffset=gameState.basketYOffset;
    applyBasketPosition();
    if(save)scheduleSave();
}

/* Two-dimensional touch/mouse drag: the basket follows the finger in both axes. */
function moveBasket(clientX,clientY){
    const rect=gameArea.getBoundingClientRect();
    basketX=clientX-rect.left-basket.offsetWidth/2;

    const centerY=clientY-rect.top;
    const baseTop=gameArea.clientHeight-basket.offsetHeight-18;
    const desiredOffset=centerY-(baseTop+basket.offsetHeight/2);
    setBasketVerticalOffset(desiredOffset,false);
    applyBasketPosition();
}
...
document.addEventListener("keydown",event=>{
    ...
    if(event.key==="ArrowUp"||event.key.toLowerCase()==="w"){
        setBasketVerticalOffset(gameState.basketYOffset-15);
        event.preventDefault();
    }
...
function purchaseOrEquipSkin(skinId){
    ...
    if(playerData.coins<item.price){
        showMessage("🪙","Not Enough Coins",`You need ${formatNumber(Math.max(0,item.price-playerData.coins))} more coins.`);
        return;
    }
...
function clearIndexedDBSave(){
    try{
        if(!window.indexedDB)return;
        const request=indexedDB.open("CoinCatcherDB",1);
        request.onsuccess=()=>{
            const db=request.result;
            if(!db.objectStoreNames.contains("save")){db.close();return;}
            try{
                const tx=db.transaction("save","readwrite");
                tx.objectStore("save").delete("playerData");
                tx.oncomplete=()=>db.close();
                tx.onerror=()=>db.close();
            }catch(error){db.close();}
        };
    }catch(error){
        console.warn("IndexedDB clear unavailable:",error);
    }
}
...
document.addEventListener("visibilitychange",()=>{
    savePlayerData();
    if(document.hidden&&gameState.running&&!gameState.paused)pauseGame();
});
