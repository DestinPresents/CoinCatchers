...
function savePlayerData(){
    try{
        ...
        // Persistent IndexedDB backup for WebView/PWA environments.
        if(window.indexedDB){
...
function restoreFromIndexedDBIfNeeded(){
    ...
    request.onsuccess=event=>{
        ...
        try{
            ...
            get.onsuccess=()=>{
                if(!hasLocal && get.result){
                    try{
                        const restored=deepMerge(DEFAULT_DATA,JSON.parse(get.result));
                        playerData=restored;
                        savePlayerData();
                        updateAllUI();
                        applyCurrentSkin();
...
function getDifficulty(level){
    ...
    if(level<=10){
        speedMultiplier=0.75+level*0.025;
        spawnInterval=1050-level*12;
        maxObjects=2;
        bombChance=2;
    }else if(level<=20){
        speedMultiplier=1+(level-10)*0.045;
        spawnInterval=900-(level-10)*15;
        maxObjects=3;
        bombChance=4;
    }else if(level<=40){
        speedMultiplier=1.45+(level-20)*0.035;
        spawnInterval=750-(level-20)*8;
        maxObjects=4;
        bombChance=8+(level-20)*0.3;
    }else if(level<=50){
        speedMultiplier=2.15+(level-40)*0.055;
        spawnInterval=590-(level-40)*7;
        maxObjects=5;
        bombChance=15+(level-40)*0.7;
    }else if(level<=70){
        speedMultiplier=2.7+(level-50)*0.065;
        spawnInterval=500-(level-50)*5;
        maxObjects=6;
        bombChance=22+(level-50)*0.8;
    }else if(level<=90){
        speedMultiplier=4+(level-70)*0.075;
        spawnInterval=390-(level-70)*4;
        maxObjects=7;
        bombChance=30+(level-70)*0.7;
    }else{
        speedMultiplier=5.5+(level-90)*0.09;
        spawnInterval=310-(level-90)*3;
        maxObjects=8;
        bombChance=44+(level-90)*0.8;
    }
    ...
    return{
        ...
        maxObjects,
        bombChance:Math.min(65,bombChance)
    };
}
...
function spawnObject(){
    ...
    // Only some normal coins are deliberately faster.
    // Other objects and normal coins keep their normal speed.
    const isFastCoin = type==="coin" && Math.random()<0.18;
    const speedMultiplier = isFastCoin
        ? randomBetween(1.38,1.58)
        : randomBetween(.9,1.12);

    if(isFastCoin)object.classList.add("fast-coin");
    ...
    gameState.objects.push({
        ...
        x,
        y:startY,
        speed:GAME_CONFIG.BASE_FALL_SPEED*difficulty.speedMultiplier*speedMultiplier,
...
function getBasketVerticalLimits(){
    ...
    const minY=Math.max(0,areaHeight*0.50-basketHeight);
    const maxY=Math.max(minY,areaHeight-basketHeight-8);
    return {minY,maxY};
}
...
function resetBasketPosition(){
    ...
    if(Number.isFinite(savedY)){
        ...
        basketY=Math.max(0,height-basket.offsetHeight-28);
    }
...
gameArea.addEventListener("pointerleave",()=>pointerActive=false);
...
function ensureAchievementToastStyles(){
    if(document.getElementById("achievementToastStyles"))return;

    const style=document.createElement("style");
    style.id="achievementToastStyles";
    style.textContent=`
        ...
        .achievement-toast{
            ...
            opacity:0;
            animation:ccToastIn .25s ease-out forwards;
        }
        .achievement-toast.out{
            animation:ccToastOut .25s ease-in forwards;
        }
...
function processAchievementToastQueue(){
    ...
    reward.className="achievement-toast-reward";
    ...
    // Deliberately no sound and no modal/overlay. Gameplay continues underneath.
    setTimeout(()=>{
...
// Periodic backup protects progress if a WebView is closed unexpectedly.
...
/* Fast coins use the same normal coin appearance; this class is only a hook. */
