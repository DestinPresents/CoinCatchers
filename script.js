...
const IDB_NAME="CoinCatcherDB";
const IDB_STORE="gameData";
const IDB_KEY="playerData";

function saveIndexedDBBackup(){
    if(!window.indexedDB)return;
    try{
        const request=indexedDB.open(IDB_NAME,1);
        request.onupgradeneeded=e=>{if(!e.target.result.objectStoreNames.contains(IDB_STORE))e.target.result.createObjectStore(IDB_STORE);};
        request.onsuccess=e=>{
            const db=e.target.result;
            try{const tx=db.transaction(IDB_STORE,"readwrite");tx.objectStore(IDB_STORE).put(JSON.stringify(playerData),IDB_KEY);tx.oncomplete=()=>db.close();tx.onerror=()=>db.close();}catch(_){try{db.close()}catch(e){}}
        };
    }catch(_){}
}
("Save failed:",error);
...
function startGame(){
    ...
    updateAllUI();
    startGameLoop();
    const initialCount=Math.min(getDifficulty(gameState.level).maxObjects,3);
    for(let i=0;i<initialCount;i++){
        setTimeout(()=>spawnObject(),i*140);
    }
    scheduleNextSpawn();
}
...
function getDifficulty(level){
    ...
    if(level<=5){
        speedMultiplier=0.80; spawnInterval=1050; maxObjects=3; bombChance=4;
    }else if(level<=10){
        speedMultiplier=0.90; spawnInterval=1000; maxObjects=3; bombChance=7;
    }else if(level<=20){
        speedMultiplier=1.00; spawnInterval=900; maxObjects=3; bombChance=10;
    }else if(level<=30){
        speedMultiplier=1.20; spawnInterval=820; maxObjects=4; bombChance=13;
    }else if(level<=40){
        speedMultiplier=1.40; spawnInterval=750; maxObjects=4; bombChance=16;
    }else if(level<=50){
        speedMultiplier=1.65; spawnInterval=670; maxObjects=5; bombChance=20;
    }else if(level<=60){
        speedMultiplier=1.90; spawnInterval=590; maxObjects=5; bombChance=24;
    }else if(level<=70){
        speedMultiplier=2.20; spawnInterval=510; maxObjects=6; bombChance=28;
    }else if(level<=80){
        speedMultiplier=2.55; spawnInterval=430; maxObjects=7; bombChance=35;
    }else if(level<=90){
        speedMultiplier=2.90; spawnInterval=350; maxObjects=8; bombChance=42;
    }else{
        speedMultiplier=3.25; spawnInterval=280; maxObjects=10; bombChance=50;
    }

    return {
        speedMultiplier,
        spawnInterval:Math.max(180,spawnInterval),
        maxObjects,
        bombChance
    };
}
...
function spawnObject(){
    ...
    gameState.objects.push({
        ...
        x,
        y:startY,
        speed:GAME_CONFIG.BASE_FALL_SPEED*difficulty.speedMultiplier*
            (type === "coin" && Math.random() < 0.18 ? randomBetween(1.38,1.58) : randomBetween(.92,1.08)),
...
function handleObjectMiss(obj){
    if(!obj || obj.type === "heart" || obj.type === "bomb") return;

    const penalty = (obj.type === "diamond" || obj.type === "golden") ? 15 : 10;
...
function resetBasketPosition(){
    ...
    const minY=Math.max(0,gameArea.clientHeight*0.50-basket.offsetHeight);
    const maxY=Math.max(minY,gameArea.clientHeight-basket.offsetHeight-8);
    basketY=Number.isFinite(savedY)?savedY:maxY;
...
function resetProgress(){
    ...
    if(window.indexedDB){try{const r=indexedDB.open(IDB_NAME,1);r.onsuccess=e=>{const db=e.target.result;try{const tx=db.transaction(IDB_STORE,"readwrite");tx.objectStore(IDB_STORE).delete(IDB_KEY);tx.oncomplete=()=>db.close();}catch(_){try{db.close()}catch(e){}}};}catch(_){}}
...
function showAchievementToast(achievement){
    ...
    if(!container){
        ...
        style.textContent=`
            .achievement-toast-container{position:fixed;top:12px;right:12px;z-index:99999;width:min(300px,calc(100vw - 24px));display:flex;flex-direction:column;gap:8px;pointer-events:none;}
            .achievement-toast{display:flex;align-items:center;gap:10px;padding:11px 13px;border:1px solid rgba(245,197,66,.35);border-radius:14px;background:rgba(14,15,20,.95);box-shadow:0 10px 28px rgba(0,0,0,.35);backdrop-filter:blur(12px);transform:translateX(calc(100% + 20px));opacity:0;animation:ccToastIn .25s ease-out forwards;}
            .achievement-toast.out{animation:ccToastOut .25s ease-in forwards;}
            .achievement-toast-icon{width:38px;height:38px;flex:0 0 38px;display:grid;place-items:center;border-radius:11px;background:rgba(245,197,66,.1);font-size:20px;}
            .achievement-toast-title{color:#f5c542;font-size:10px;font-weight:800;letter-spacing:.5px;text-transform:uppercase;}
            .achievement-toast-name{color:#fff;font-size:13px;font-weight:700;margin-top:2px;}
            .achievement-toast-reward{color:#aeb1bb;font-size:10px;margin-top:2px;}
            @keyframes ccToastIn{to{transform:translateX(0);opacity:1}}
            @keyframes ccToastOut{to{transform:translateX(calc(100% + 20px));opacity:0}}
            @media(max-width:600px){.achievement-toast-container{top:8px;right:8px;width:min(275px,calc(100vw - 16px));}}`;
...
function processAchievementToastQueue(){
    ...
    toast.innerHTML=`<div class="achievement-toast-icon">${achievement.icon||"🏆"}</div><div><div class="achievement-toast-title">Achievement Unlocked</div><div class="achievement-toast-name"></div><div class="achievement-toast-reward"></div></div>`;
    toast.querySelector(".achievement-toast-name").textContent=achievement.title||"Achievement";
    toast.querySelector(".achievement-toast-reward").textContent=`+${formatNumber(achievement.reward||0)} Coins`;
    document.getElementById("achievementToastContainer").appendChild(toast);
    setTimeout(()=>{
        toast.classList.add("out");
        setTimeout(()=>{toast.remove();achievementToastActive=false;processAchievementToastQueue();},260);
    },2300);
}
